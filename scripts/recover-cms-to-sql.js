...
const STORAGE_BUCKET = process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET || 'myreatstat.firebasestorage.app';
...
async function fetchCsvText(file) {
  // Direct SQL imports store the raw CSV inline with no storage object.
  if (file.rawContent && !file.storageUrl && !file.storagePath) {
    return file.rawContent;
  }
  const url = file.storageUrl || storageUrlFromPath(file.storagePath);
  if (!url) return null;
...
(async () => {
  ...
  for (const file of files) {
    if (!file.storageUrl && !file.storagePath && !file.rawContent) {
      console.log(`SKIP (no source): ${file.name}`);
      skippedFiles++;
      continue;
    }
    try {
      const csvText = await fetchCsvText(file);
      if (csvText == null) {
        console.error(`FETCH_FAIL ${file.name}: no fetchable source`);
        skippedFiles++;
        continue;
      }
